...
class GeneticEngine {
  ...
  // returns {id, phenotype, parents, operation, limit}
  buildPhenotypeRec () {
  ...
  /**
   * limit  : Minimal fitness that is accepted for the phenotype, 
   *          it is qual to max of fitness of parents. There is no
   *          sense to add to population phenotype with fitness less than parents.
   * parents: Array of parents that was used for the phenotype.
   *          the list of phenotypes will be removed from population
   *          if phenotype fitness is accepted in rank function
  */
  getPhenotype () {
    let { id, phenotype, parents, operation, limit } = this.buildPhenotypeRec()
    this.evaluation.set(id, { phenotype, parents, limit })
    return { id, phenotype, operation }
  }
